...
export class EcitizenGateway {
  ...
  public url: string = 'https://payments.ecitizen.go.ke/PaymentAPI/iframev2.1.php';
  public statusUrl: string = '';
  public pictureURL: string = '';
  public currency: string = 'KES';
  public sendSTK: boolean = false;
  public successStatuses: string[] = ['paid', 'settled', 'success', 'successful', 'completed', 'complete'];

  constructor(config?: Partial<EcitizenConfig>) {
    if (config && Object.keys(config).length > 0) {
      if (config.apiClientID !== undefined) this.apiClientID = String(config.apiClientID);
      if (config.apiKey !== undefined) this.apiKey = String(config.apiKey);
      if (config.secret !== undefined) this.secret = String(config.secret);
      if (config.serviceID !== undefined) this.serviceID = String(config.serviceID);
      if (config.url !== undefined) this.url = String(config.url);
      if (config.statusUrl !== undefined) this.statusUrl = String(config.statusUrl);
      if (config.pictureURL !== undefined) this.pictureURL = String(config.pictureURL);
      if (config.currency !== undefined) this.currency = String(config.currency);
      if (config.sendSTK !== undefined) this.sendSTK = Boolean(config.sendSTK);
  ...
  public assertConfigured(): void {
    const required: Array<keyof Pick<this, 'apiClientID' | 'apiKey' | 'secret' | 'serviceID' | 'url'>> = [
      'apiClientID',
      'apiKey',
      'secret',
      'serviceID',
   ...
   */
  public createCheckoutPayload(invoice: Record<string, any>): Record<string, string> {
    this.assertConfigured();
   ...
   */
  public generateCheckoutHash(payload: Record<string, any>): string {
    this.assertConfigured();

    const dataString =
      String(this.apiClientID) +
   ...
   */
  public verifyNotificationHash(payload: Record<string, any>): boolean {
    this.assertConfigured();
    ...
    const dataString =
      String(payload.client_invoice_ref ?? payload.billRefNumber ?? '') +
   ...
   */
  public isSuccessStatus(status: any): boolean {
  ...
  public normalizeAmount(amount: any): string {
   ...
   */
  private hmacSha256HexThenBase64(dataString: string): string {
