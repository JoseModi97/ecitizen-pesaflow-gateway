...
function parseFlags(args: string[]): { flags: Record<string, string>; booleans: Set<string> } {
  const flags: Record<string, string> = {};
...
async function runPayCommand(args: string[]): Promise<void> {
  console.log(BANNER);
  ...
  try {
    ...
    if (flags['email']) payment.email = flags['email'];
    if (flags['currency']) payment.currency = flags['currency'];
    if (flags['callback-url']) payment.callbackUrl = flags['callback-url'];
    if (flags['notify-url']) payment.notifyUrl = flags['notify-url'];
    ...
    if (!autoYes) {
      ...
      const proceed = await confirmPrompter.confirm(
        '\nSubmit this payment to the LIVE eCitizen PaymentAPI now?',
        false
      );
 ...
 */
function startCheckoutServer(pageHtml: string): Promise<{ server: http.Server; url: string }> {
...
): Promise<'settled' | 'timeout' | 'interrupted'> {
