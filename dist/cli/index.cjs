...
async function runInitWizard(args) {
    ...
    const envInfo = scaffolder_1.ProjectScaffolder.detectProjectEnvironment(targetDir);
    ...
    try {
        ...
        // 1. Update .env
        const envFile = scaffolder_1.ProjectScaffolder.updateEnvFile(targetDir, answers);
        console.log(`  \x1b[32m✔\x1b[0m Configured environment: \x1b[1m${path.basename(envFile)}\x1b[0m`);
        // 2. Create config file
        const configFile = scaffolder_1.ProjectScaffolder.createConfigFile(targetDir, answers);
        console.log(`  \x1b[32m✔\x1b[0m Created client config: \x1b[1m${path.basename(configFile)}\x1b[0m`);
        // 3. Scaffold controller
        const controllerFiles = scaffolder_1.ProjectScaffolder.scaffoldController(targetDir, answers);
...
async function runPayCommand(args) {
    ...
    try {
        ...
        if (!autoYes) {
            const confirmPrompter = new prompt_1.CliPrompter();
...
async function runTestCommand() {
    ...
    try {
        const gateway = new gateway_1.EcitizenGateway({
            apiClientID: 'CLIENT1',
            apiKey: 'KEY1',
            secret: 'SECRET1',
            serviceID: 'SERVICE1',
